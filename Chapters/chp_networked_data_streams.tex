...
\end{enumerate}
In this chapter, we present two solutions to this problem, each with its own advantages and disadvantages. Both are inspired by the concept of sparse representations, which expresses a signal/model $f$, defined over an independent variable $x$, as a linear combination of a few atoms from a pre-specified and overcomplete dictionary of size $m$:
\begin{equation}
...
\end{itemize}
Thus, $\Theta \triangleq \{(\vpi^{(z)}, \mA^{(z)}, \mB^{(z)})\}_{z=1}^m$, where each triplet $(\vpi^{(z)}, \mA^{(z)}, \mB^{(z)})$ completely defines one HMM in the lattice. The SOHMMM online learning algorithm corresponds to stochastic gradient descent over the energy function~\plaineqref{eq:sohmmm_energy}. Constrained optimization is avoided by reparameterizing the model using softmax functions, so that standard, unconstrained stochastic gradient descent can be performed over the new parameters $\vu^{(z)}$, $\mW^{(z)}$, and $\mR^{(z)}$:
\begin{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.65]{ChapterThree/lattice-rect.png}
...
\begin{equation}
    ...
    \label{eq:hmm_div}
\end{equation}
where $\mX$ has length $T$ and is sampled from $p(\rmX \mid z)$. \Eqref{eq:hmm_div} is a Monte Carlo approximation of the KL divergence between two HMMs and therefore measures how well model $z'$ matches observations generated by model $z$, relative to how well model $z$ matches observations generated by itself. Since this relationship is asymmetric, we use the following symmetrized version instead:
\begin{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.25]{ChapterThree/hmm_ref.png}
...
We compared the SOHMMM algorithm to two other approaches: i) hidden Markov model initialized with universal background model (HMM-UBM), addressed in detail by \citet{Anisa2017}, and ii) the SOHMMM algorithm with a zero neighborhood, which utilizes the incremental-learning part of the SOHMMM algorithm, without updating the HMMs in the vicinity (Z-SOHMMM).
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.8]{ChapterThree/wifi_network.png}
