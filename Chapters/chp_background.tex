...
This chapter will then focus on providing a brief yet comprehensive background on the aforementioned subject. We start by clarifying some of the notations and conventions we will adopt (\Secref{sec:definitions}). We then proceed with a short introduction to Bayesian networks, motivated by the factorization properties of joint probability functions (\Secref{sec:bayesian_networks}). Under this subject, the hidden Markov model is highlighted (\Secref{sec:hmm}) as a particular case since this model will play a central role in Chapter \ref{chp:networked_data_streams}. Expectation-maximization is presented as an efficient algorithm for learning the parameters of probabilistic models with unobserved variables (\Secref{sec:expectation_maximization}) and the procedure is illustrated using the hidden Markov model as an example (\Secref{sec:em_hmm}). The procedure is then extended to the variational setting and the variational autoencoder is introduced (\Secref{sec:variational_autoencoder}) since it will be one of the models employed in Chapter \ref{chp:domain_generalization}.
...
\end{align}
The three equivalent factorizations \plaineqref{eq:chain} -- \plaineqref{eq:reversed_chain} correspond to the three Bayesian networks in \Figref{fig:equiv_bayesian_nets}, respectively from left to right, and constitute a Markov equivalence class.
