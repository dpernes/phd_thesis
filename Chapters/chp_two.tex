...
We implemented a set of wireless simulations using the OMNeT++ \cite{Ref27} simulator along with the INET framework \cite{Ref28}. OMNeT++ is a C++-based discrete-event simulator (DES) for modeling communication networks, multiprocessors, and other distributed or parallel systems.  
...
Table \ref{tbl:sohmmm_results} presents the results for anomaly detection in the aforementioned scenarios. In this table, the AUC values and F1 scores of each anomalous scenario are presented for the HMM-UBM, Z-SOHMMM, and SOHMMM models. The anomalous APs are also indicated. This table summarizes the information of Figures \ref{Fig5:4} to \ref{Fig5:11} and includes the F1 score for a comparative analysis. The ROC curves in the aforementioned figures demonstrate that the SOHMMM outperforms the remaining models since it is better at discriminating normal and anomalous samples, which is confirmed by the higher AUC values. The F1 score, on the other hand, is a measure of the accuracy, considering both the precision and recall to compute the score. The obtained values for the F1 score once again confirm the superiority of the SOHMMM algorithm.
...
\begin{enumerate}
	\item the $m$-dimensional mixture coefficients, $\valpha^{(y)}$, where $ \valpha^{(y)}_z \triangleq p(\rz=z \mid \ry=y)$, for $z \in \{1,\dots,m\}$ and $y \in \{ 1,\dots,k \}$;
	\item the $s$-dimensional initial state probabilities, $\vpi^{(z)}$, where $\evpi_h^{(z)} \triangleq p(\rh^{(0)}=h \mid \rz=z)$, for $h \in \{1,\dots,s\}$ and $z \in \{1,\dots,m\}$;
	\item the $s \times s$ state transition matrices, $\mA^{(z)}$, where $\emA^{(z)}_{h,h'} \triangleq p(\rh^{(t)}=h' \mid \rh^{(t-1)}=h, \rz=z)$, for $h,h' \in \{1,\dots,s\}$ and $z \in \{1,\dots,m\}$;
...
&= \frac{1}{n}\sum_{i=1}^n \log p(\mX_i \mid y_i, \theta) + \frac{\reg}{2} \sum_{\substack{y,y'=1,\\y'\neq y}}^{k} \emG_{y,y'} {\valpha^{(y)}}\transp \valpha^{(y')},
\end{align}
...
\begin{algorithm}
	...
	\label{alg:mhmm}
	\begin{algorithmic}
		\State Inputs: The training set, consisting of $n$ tuples $(\mX_i,y_i)$, a set of initial parameters $\Theta^{(0)}$ and the number of training iterations \texttt{trainIter}.
		\For{$j = 1, \dots,$ \texttt{trainIter}} $\quad$ (computing sufficient statistics):
		\begin{enumerate}
			\item $n_k \coloneqq \sum_i \1_{y_i=y}$ for $y=1,\dots,k$.
			\item Obtain the mixture posteriors $\eta^{(z)}_{i} \coloneqq p(z \mid \mX_i, y_i)$, for $i=1,\dots,n$ and $z=1,\dots,m$, by computing  $\tilde{\eta}^{(z)}_{i} \coloneqq p(\mX_i \mid z)p(z \mid y_i)$ and normalizing it.
			\item Obtain the state posteriors $\gamma^{(z)}_{i,h}(\tau) \coloneqq p(\rh^{(\tau)}=h \mid z, \mX_i)$ and $\xi^{(z)}_{i,h,h'}(\tau) \coloneqq p(\rh^{(t-1)}=h, \rh^{(t)}=h' \mid z, \mX_i)$, for $i=1,\dots,n$, $\tau=1,\dots,t_i$, $z=1,\dots,m$ and $h,h'=1,\dots,s$, as done in the Baum-Welch algorithm (\citet{Baum1972}).
		\end{enumerate}
		\State M-step:
		\begin{enumerate}
			\item $\evalpha^{(y)}_{z} := \frac{\sum_i \eta^{(z)}_{i} \1_{y_i=y}}{n_y}$, for $y=1,\dots,k$ and $z=1,\dots,m$, obtaining $\valpha_y$.
			\item $\evpi^{(z)}_h := \frac{\sum_i \eta^{(z)}_{i} \gamma^{(z)}_{i,h}(0)}{\sum_i \eta^{(z)}_{i}}$, for $z=1,\dots,m$ and $h=1,\dots,s$, obtaining $\vpi^{(z)}$. 
			\item $\emA^{(z)}_{h,h'} = \frac{\sum_i \eta^{(z)}_{i} \sum_{\tau=1}^{t_i} \xi^{(z)}_{i,h,h'}(\tau)}{\sum_i \eta^{(z)}_{i} \sum_{\tau=0}^{t_i-1} \gamma^{(z)}_{i,h}(\tau)}$, for $z=1,\dots,m$ and $h,h'=1,\dots,s$, obtaining $\mA^{(z)}$.
			\item $\vmu^{(z)}_h = \frac{\sum_i \eta^{(z)}_{i} \sum_{\tau=1}^{t_i} \gamma^{(z)}_{i,h}(\tau) \vx_i^{(\tau)}} {\sum_i \eta^{(z)}_{i} \sum_{\tau=1}^{t_i} \gamma^{(z)}_{i,h}(\tau)}$, for $z=1,\dots,m$ and $h=1,\dots,s$.
